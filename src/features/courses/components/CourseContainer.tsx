...
const CourseContainer: React.FC<CourseContainerProps> = ({ course }) => {
    ...
    return (
        <div className={`rounded-3xl border-2 overflow-hidden transition-all duration-500 ${isExpanded
            ? 'border-[var(--accent-primary)]/50 dark:shadow-2xl dark:shadow-purple-500/20 shadow-2xl shadow-purple-200'
            ...
            <div
                ...
                className="cursor-pointer w-full p-5 hover:bg-[var(--bg-secondary)] transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-[var(--accent-primary)] rounded-3xl"
            >
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-8">
                    ...
                    <div className="flex items-start md:items-center gap-4 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <h3 className="text-lg md:text-xl font-bold truncate pr-2 text-[var(--text-main)]">
                                {course.title}
                            </h3>
                            {course.short_description && (
                                <p className="text-sm line-clamp-1 md:line-clamp-1 text-[var(--text-secondary)]">
                                    {course.short_description}
                                </p>
                            )}
                        </div>
                    ...
                    <div className="flex items-center justify-between md:justify-end gap-3 md:gap-6 w-full md:w-auto mt-2 md:mt-0 pt-3 md:pt-0 border-t md:border-none border-dashed border-[var(--border-main)]">
                        ...
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${isExpanded ? 'bg-[var(--accent-primary)] text-white rotate-180' : 'bg-[var(--bg-muted)] text-[var(--text-secondary)] hover:text-[var(--accent-primary)]'}`}>
                ...
                {/* Progress Bar */}
                {completedModules.size > 0 && (
                    <div className="mt-4 md:mt-0 absolute bottom-0 left-0 right-0 h-1 bg-[var(--bg-muted)]">
            ...
            {isExpanded && (
                <div className="border-t border-[var(--border-main)]">
                    <div className="px-6 md:px-8 py-4 dark:bg-gradient-to-r dark:from-purple-900/10 dark:to-violet-900/10 bg-gradient-to-r from-purple-50/50 to-violet-50/50">
                    ...
                    <div className="p-6 md:p-8 space-y-4">
                        {daysLoading ? (
                            ...
                            <p className="text-center text-[var(--text-muted)] py-8">No modules available yet.</p>
                        ) : (
                            days.map((day, index) => (
                                <ModuleAccordion
                                    key={day.id}
                    ...
                    {dayCount > 0 && completedModules.size === dayCount && (
                        <div className="mx-6 md:mx-8 mb-6 md:mb-8 p-6 rounded-3xl text-center dark:bg-gradient-to-r dark:from-emerald-900/30 dark:to-green-900/30 dark:border dark:border-emerald-500/30 bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200">
                            ...
                            <h4 className="text-xl font-bold mb-1 text-emerald-700 dark:text-emerald-400">Course Complete!</h4>
                            <p className="text-sm text-slate-600 dark:text-slate-400">You've completed all modules. Ready for the next course?</p>
                        </div>
                    )}
                </div>
            )}
